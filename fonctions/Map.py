...
def make_case(rect, couleur, owner, cell, taille):
    surface = pg.Surface((taille, taille))
    surface.fill(couleur)
    return (rect, surface, owner, cell)
...
def create_map(screen):
    ...
    def place_zone(color, size, zone_min_col=None, zone_max_col=None, attempts_limit=1000):
        ...
        while attempts<attempts_limit:
            ...
            j=random.randint(zone_min_col if zone_min_col else 0, zone_max_col if zone_max_col else colonne-1)
            ...
            while len(zone)<size:
                ...
                grid[ni][nj]=color
                zone.append((ni,nj))
            if len(zone)>=size: return True
            for x,y in zone: grid[x][y]=None
        return False
    ...
    pos_j1 = place_player(joueur_01,1,18,1,3)
    pos_j2 = place_player(joueur_02,1,18,16,18)
    ...
    add_unite(2,pos_j2,[(-1,0),(0,-1),(1,0)])

    # case_original = rect, texture, owner, couleur cellule
    case_original=[]
    ...
    return BCOLOR, case_original, pos_j1, pos_j2, textures, taille, offset_x, offset_y, grid_points, joueurs_data, spawn_zone_1, spawn_zone_2, unites, grid
...
def handle_click(mouse_pos, case_original, joueur_id, joueurs, taille, offset_x, offset_y, grid_points, joueurs_data):
    ...
    for idx, (rect, couleur, owner, cell) in enumerate(case_original):
        if rect.collidepoint(mouse_pos):
            ...
            case_original[idx] = make_case(rect, new_color, joueur_id, cell, taille)
            ...
            for idx, (i, j) in enumerate(positions):
                ...
                if rect_unit.collidepoint(mouse_x, mouse_y):
                    if interface.joueurs[joueur_actif]["tickets"] >= 1:
                        ...
                        forbidden = [textures["eau"], textures["roche"], textures["foret"], textures["sable"]]
...
def update_unit_animation(unites, interface, case_original, grid_points, taille, offset_x, offset_y):
    ...
    if animating_move:
        ...
        if step < len(path)-1:
            ...
            rect, couleur, owner, cell = case_original[idx_case]
            if owner != joueur_id:
                case_original[idx_case] = make_case(rect, interface.joueurs[joueur_id]["color"], joueur_id, cell, taille)
...
def make_case(rect, couleur, owner, cell, taille):
    """Retourne un tuple (rect, surface, owner, cell) pour case_original"""
    surface = pg.Surface((taille, taille))
    surface.fill(couleur)
    return (rect, surface, owner, cell)
